...
const BottomNav = () => {
  ...
  // 컴포넌트가 마운트되거나 페이지 이동(location)이 있을 때마다 로그인 상태 체크
  useEffect(() => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 py-2 pb-safe bg-white dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800 shadow-[0_-4px_12px_rgba(26,65,115,0.08)]">
      ...
      {isLoggedIn ? (
        ...
        <Link className="flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 hover:text-[#1A4173] dark:hover:text-blue-300 transition-transform duration-150 scale-95" to="/login">
